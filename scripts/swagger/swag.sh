...
./scripts/swagger/echo-swag/swag init -g api/router.go --parseDependency --parseInternal --parseDepth 1  --exclude cmd/,api/v2/ --output docs/swagger

## workaround to add the security and host - this so we can add the Bearer token to the header
## Json 
jq '.host = "'${host}'"' ./docs/swagger/swagger.json > ./docs/swagger/swagger_host.json
jq '."securityDefinitions"={"bearerAuth":{"type":"apiKey","name":"Authorization","in":"header"}}' ./docs/swagger/swagger_host.json > ./docs/swagger/swagger_security.json
jq '."security"=[{"bearerAuth":[]}]' ./docs/swagger/swagger_security.json > ./docs/swagger/swagger_temp.json

## yaml
./scripts/swagger/echo-swag/yq e '.host = "'${host}'"' ./docs/swagger/swagger.yaml > ./docs/swagger/swagger_host.yaml
./scripts/swagger/echo-swag/yq e '."securityDefinitions"={"bearerAuth":{"type":"apiKey","name":"Authorization","in":"header"}}' ./docs/swagger/swagger_host.yaml > ./docs/swagger/swagger_security.yaml
./scripts/swagger/echo-swag/yq e '."security"=[{"bearerAuth":[]}]' ./docs/swagger/swagger_security.yaml > ./docs/swagger/swagger_temp.yaml

##  Move the files back
mv ./docs/swagger/swagger_temp.json ./docs/swagger/swagger.json
mv ./docs/swagger/swagger_temp.yaml ./docs/swagger/swagger.yaml
